 ...
 * Shows a small icon in the top-left corner of each card.
 *
 * Version: 1.0.0
 */
(function () {
  ...
  function injectStyles() {
    ...
    style.id = 'dt-category-icons-style';
    style.textContent = CSS;
    document.head.appendChild(style);
  }

  function getCategory(el) {
    // data-category attribute auf dem card element
    const cat = el.dataset.category?.toLowerCase().trim();
    if (cat && ICONS[cat]) return cat;
    // Fallback: ncf-category class
    const classes = el.className.split(' ');
    for (const cls of classes) {
      if (cls.startsWith('ncf-category-')) {
        const fromClass = cls.replace('ncf-category-', '').split('__')[0].toLowerCase();
        if (ICONS[fromClass]) return fromClass;
      }
    }
    return null;
  }
  ...
  function addIcons() {
    document.querySelectorAll('.cru-ncf-map-list-item').forEach(card => {
      if (card.querySelector('.dt-category-icon')) return; // bereits gesetzt
  ...
  function init() {
    injectStyles();
    addIcons();
    // MutationObserver für nachgeladene Karten (Filter, Scroll)
    const list = document.querySelector('.cru-ncf-map-list, [class*="ncf-map-list"]');
    if (list) {
      new MutationObserver(() => addIcons()).observe(list, { childList: true, subtree: true });
    }
  }
